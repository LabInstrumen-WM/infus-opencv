...
if(tombol_kanan||tombol_kiri)
	{	
		...
		if (equalizeFlag) equalizeHist(gray,gray);
		medianBlur(gray,filter,5);
		value=(double)threshold(filter,otsu,0,255,CV_THRESH_BINARY | CV_THRESH_OTSU);
		...
		for(size_t i=0; i<lines.size(); i++)
		{
			...
			for(int i=0; i<=15; i++)
			{
				...
				{
					if((l[0]<=batas_kanan+15 && l[0]>=batas_kanan-10) and baca)
					{
					...
					if((l[0]>=batas_kiri-20 && l[0]<=batas_kiri+25)and (not baca))
					{
						line(frame,Point(l[0],l[1]),Point(l[2],l[3]), Scalar(255,0,0),3,CV_AA);
						index_h1=l[0];
					}
			...
			for(int j=0;j<=batas_miring;j++)
			{
				...
				{
					if((l[0]<=index_h+25 and baca)||(l[0]>=index_h1-60 and (not baca)))
					{
		...
		if((key=='a')||(key=='A')) brightness=brightness+0.05;
		if((key=='z')||(key=='Z')) brightness=brightness-0.05;
		if((key=='s')||(key=='S'))  imwrite("/home/pi/frame.jpg",frame);
		if((key=='c')||(key=='C'))  
		{
		...
		if((key=='l')||(key=='L')) template_img=imread("/home/pi/frame.jpg",0);
		if((key=='w')||(key=='W')) addwFlag^=1;
		if((key=='e')||(key=='E')) equalizeFlag^=1;
		if((key=='m')||(key=='M')) infusFlag^=1;
		if((key=='r')||(key=='R')) 
		{
			batas_kiri=0;
			batas_kanan=0;
